...
8. All chart colors and metric ordering follow ux:visual-system constants (C_ACV, C_BUDGET, C_CONSUMED, C_PROJ_CONSUMED, C_PCT); Consumed → Budget → ACV order enforced on all surfaces (top nav, industry bar, customer cards, L1 tiles, donut cards, L3 drawer tabs); top nav header shows "YTD Consumed $x · YTD Budget $x · YTD ACV $x | Proj Consumed $x · Proj Budget $x · Proj ACV $x" with labels in #64748b and values in semantic colors; no RAG/threshold coloring; language standard enforced: "consumed" not "actuals", "budget" not "target".
