...
8. All chart colors and metric ordering follow ux:visual-system constants (C_ACV, C_BUDGET, C_CONSUMED, C_PROJ_CONSUMED, C_PCT); Consumed → Budget → ACV order enforced on all surfaces (top nav, industry bar, customer cards, L1 tiles, donut cards, L3 drawer tabs); industry header strip shows 8 metrics: 6 dollar metrics (YTD Consumed · YTD Budget · YTD ACV · Proj Consumed · Proj Budget · Proj ACV) plus YTD Att% and Proj Att% (both in #0f172a black, NOT C_PCT); YTD Att% computed via attPct() rounded with Math.ceil (rounds up to next integer); top nav uses dark background #0f172a with white title text and #94a3b8 metric labels; active tab underline is #8ecae6 (ACV color); industry tab bar and customer tab bar use dark teal #0e7490 background — active tab state shows #e0f2fe background with #0e7490 text and 3px bottom accent, inactive tabs show rgba(255,255,255,0.8) text; industry panel and customer panel each have a light teal #e0f2fe subheader band flush under the tab bar (border-bottom: 1px solid #bae6fd) with entity name in #0e7490 bold 22px; industry tab buttons show a projected ACV sub-line below the industry name in #8ecae6 at font-size 10px font-weight 400; all metric value numbers are bold (font-weight:700) throughout all surfaces; top nav header shows "YTD Consumed $x · YTD Budget $x · YTD ACV $x | Proj Consumed $x · Proj Budget $x · Proj ACV $x" with labels in #94a3b8 and values in semantic colors; no RAG/threshold coloring; language standard enforced: "consumed" not "actuals", "budget" not "target".
...
| 4.7.0 | 2026-06-30 | Gap merged: Two-tone color scheme — dark nav #0f172a, dark-teal tab bars #0e7490, light-teal subheader #e0f2fe; both Att% tiles black #0f172a; YTD Att% rounding Math.ceil; projected ACV sub-line on industry tabs in #8ecae6; all metric values font-weight:700 | gap-merge |
