...
5. Each contract_month conforms to contract:contract-month-shape: ytd_annual_contract_value, ytd_budget_contract_value, ytd_consumed_contract_value; YTD variances ytd_acv_gap, ytd_budget_gap, ytd_budget_attainment (null if budget = 0); projected_annual_budget_contract_value = sum of all 12 months' ytd_budget values (future months carry real budget values); projected_annual_consumed_contract_value = (ytd_consumed_at_reporting_month / months_elapsed) * 12 where months_elapsed = last 2 digits of portfolio.reporting_month; projected_annual_acv_gap, projected_annual_budget_gap, projected_annual_budget_attainment (0 if budget = 0) — all stamped on every month record at transform time; buildL1Hierarchy() receives reportingMonth as second argument; integer-safe arithmetic per entity:contract_month rules.
...
| 4.4.0   | 2026-06-29 | Criterion 5 updated: projected_annual_consumed_contract_value changed from sum-of-12-months to extrapolation formula (ytd_consumed / months_elapsed) * 12; months_elapsed from portfolio.reporting_month; buildL1Hierarchy() now accepts reportingMonth as second arg; budget projection remains sum-of-12 (unchanged) | gap-merge |
